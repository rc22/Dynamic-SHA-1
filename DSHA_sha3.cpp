...
for ( i=0; i<3; i++ )   {
	...
	ttc=tt[i];
    if(XOR == 1) temp=(((((ch[0]+ch[1])^ch[2])+ch[3])^ch[4])+ch[5])^ch[6];
    if(R1 == 1){ temp=((temp>>36) ^ temp) & 0xfffffffff;
	temp=((temp>>18) ^ temp) & 0x3ffff;
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[7]=DSHA_ROTR64( ch[7] ,temp );
    if(G == 1) ch[2]=w[0]+(ch[0] ^ ch[1] ^ ch[2])+ttc;
//0-th round
	}
	if (dshaNumRounds >= 2) {
	...
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[6]=DSHA_ROTR64( ch[6] ,temp );
    if(G == 1) ch[1]=w[1]+((ch[7] & ch[0]) ^ ch[1] )+ttc;
...
//1-th round
	...
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[5]=DSHA_ROTR64( ch[5] ,temp );
    if(G == 1) ch[0]=w[2]+((~(ch[6] | ch[0])) | (ch[6] & (ch[7] ^ ch[0])))+ttc;
...
//2-th round
	...
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[4]=DSHA_ROTR64( ch[4] ,temp );
    if(G == 1) ch[7]=w[3]+((~(ch[5] | (ch[6] ^ ch[7]))) | (ch[5] & (~(ch[7]))))+ttc;
...
//3-th round
	...
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[3]=DSHA_ROTR64( ch[3] ,temp );
    if(G == 1) ch[6]=w[4]+(ch[4] ^ ch[5] ^ ch[6])+ttc;
...
//4-th round
	...
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[2]=DSHA_ROTR64( ch[2] ,temp );
    if(G == 1) ch[5]=w[5]+((ch[3] & ch[4]) ^ ch[5])+ttc;
...
//5-th round
	...
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[1]=DSHA_ROTR64( ch[1] ,temp );
    if(G == 1) ch[4]=w[6]+((~(ch[2] | ch[4])) | (ch[2] & (ch[3] ^ ch[4])))+ttc;
...
//6-th round
	...
	temp=((temp>>12) ^ temp) & 0xfff;
    temp=((temp>>6) ^ temp) & 0x3f;}
    if(ROT == 1) ch[0]=DSHA_ROTR64( ch[0] ,temp );
    if(G == 1) ch[3]=w[7]+((~(ch[1] | (ch[2] ^ ch[3]))) | (ch[1] & (~(ch[3]))))+ttc;
